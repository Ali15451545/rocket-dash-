...
let bodyColor = '#ffffff', lastShieldScore = 0, consecutivePassed = 0;
...
const languages = {
    en: { title: "ROCKET DASH", start: "START MISSION", resume: "RESUME", mainMenu: "MAIN MENU", skins: "SKINS", best: "BEST", hint: "TAP TO JUMP", paused: "PAUSED", failed: "FAILED", shield: "🛡️ SHIELD", warp: "⚡ WARP" },
    az: { title: "ROKET DASH", start: "MİSSİYAYA BAŞLA", resume: "DAVAM ET", mainMenu: "ANA MENYU", skins: "GÖRÜNÜŞLƏR", best: "REKORD", hint: "TOXUN: TULLAN", paused: "DAYANDIRILDI", failed: "UĞURSUZ", shield: "🛡️ QALXAN", warp: "⚡ ZAMAN" },
    tr: { title: "ROKET DASH", start: "GÖREVE BAŞLA", resume: "DEVAM ET", mainMenu: "ANA MENÜ", skins: "GÖRÜNÜMLER", best: "REKOR", hint: "DOKUN: ZIPLA", paused: "DURDURULDU", failed: "BAŞARISIZ", shield: "🛡️ KALKAN", warp: "⚡ ZAMAN" }
};
let currentLang = 'en';
...
function init() {
    ...
    pipes = []; stars = []; frame = 0; score = 0; consecutivePassed = 0;
...
// MOBILE & PC SPECIALS
function useShield(e) { if (e) e.stopPropagation(); if (hasShield && state === 'PLAYING') { shieldActive = true; hasShield = false; shieldTimer = 180; shieldUi.classList.add('hidden'); } }
function useWarp(e) { if (e) e.stopPropagation(); if (hasWarp && state === 'PLAYING') { warpActive = true; hasWarp = false; warpTimer = 180; consecutivePassed = 0; warpUi.classList.add('hidden'); } }
function togglePause() { if (state === 'PLAYING') { state = 'PAUSED'; updateLanguageUI(); resumeBtn.classList.remove('hidden'); backBtn.classList.remove('hidden'); menuBox.classList.add('show-menu'); } else if (state === 'PAUSED') resumeGame(); }

function startNewGame() { score = 0; scoreEl.innerText = "0"; init(); state = 'PLAYING'; menuBox.classList.remove('show-menu'); shieldUi.classList.add('hidden'); warpUi.classList.add('hidden'); frame = -50; }
...
function update() {
    ...
    if (state === 'PLAYING') {
        ...
        if (frame > 0 && frame % 100 === 0) {
            let gap = 195; let minT = Math.max(100, lastPipeTop - 150); let maxT = Math.min(height - gap - 100, lastPipeTop + 150);
        ...
        pipes.forEach((p, i) => {
            ...
            if (rocket.x < p.x + 60 && rocket.x + 50 > p.x && (rocket.y < p.top || rocket.y + 30 > p.top + p.gap)) {
                if (!shieldActive) { state = 'GAMEOVER'; if (score > highScore) { highScore = score; localStorage.setItem('rocketBest', score); } updateLanguageUI(); resumeBtn.classList.add('hidden'); backBtn.classList.remove('hidden'); menuBox.classList.add('show-menu'); }
            }
...
const handleAction = (e) => { if (e && e.type === 'keydown' && e.code !== 'Space') return; if (e) e.preventDefault(); if (state === 'PLAYING') { let jP = warpActive ? rocket.jump * 0.6 : rocket.jump; rocket.v = jP; } };
window.addEventListener('keydown', (e) => { if (e.code === 'Space') handleAction(e); if (e.code === 'KeyQ') useShield(); if (e.code === 'KeyE') useWarp(); if (e.code === 'Escape') togglePause(); });
